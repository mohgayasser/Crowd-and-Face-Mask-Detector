...
class myDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if not self.if_test:
           ...
           # target =tar[f"{bn}/num_annot_headpoints"] 
           
            temp = h5py.File('/content/drive/MyDrive/Gp/TestOneSample/data/Denstymap/IMG_1.h5', 'r')
            sample = {'image': image}
            if self.transform:
                sample = self.transform(sample)
            sample['density_map'] = torch.from_numpy( np.asarray(temp['density']))
